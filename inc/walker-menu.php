 ...
 */
class EnvestPro_Lite extends Walker_Nav_Menu {
	/**
	 * Start Level.
	 *
	 ...
	 */
		if ( 0 === strcasecmp( $item->attr_title, 'divider' ) && 1 === $depth ) {
			...
			if ( $args->has_children && 0 === $depth ) {
				...
				if ( in_array( 'current-menu-item', $classes ) ) {
					$atts['class']			= 'dropdown-toggle active'; 
				}else{
	...
	public static function fallback( $args ) {
		if ( current_user_can( 'edit_theme_options' ) ) {
			...
			$fb_output .= '>';
			$fb_output .= '<li><a href="' . admin_url( 'nav-menus.php' ) . '">'. __( 'Add a menu', 'envestpro-lite' ) .'</a></li>';
			$fb_output .= '</ul>';
			if ( $container ) {
				$fb_output .= '</' . $container . '>'; }
			echo $fb_output;
